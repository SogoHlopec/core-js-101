...
function getCommonDirectoryPath(pathes) {
  let result = '';
  let checkPath = pathes[0];
  let checking = true;
  while (checking) {
    if (checkPath.indexOf('/') === -1) {
      checking = false;
    }
    const currentPath = result + checkPath.slice(0, checkPath.indexOf('/') + 1);
    checkPath = checkPath.slice(checkPath.indexOf('/') + 1);
    if (pathes.every((item) => item.startsWith(currentPath))) {
      result = currentPath;
    } else {
      checking = false;
    }
...
function evaluateTicTacToePosition(position) {
  ...
  if (center) {
    if (
      (position[0][0] === center && position[2][2] === center)
